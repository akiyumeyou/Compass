...
# 会話の段階的展開（重要）【早期3段論法】
- **会話1-2**: 軽い話題のみ。現在の状況確認（仕事や日常の感想など）
- **会話3**: 共感の準備段階。優しく心配を示し始める
- **会話4-6**: ビデオ通話で本格的な共感フェーズ
- **会話7-8**: 気づきフェーズ（核心に迫る）
- **会話9以降**: 行動フェーズ（約束を引き出す）
...
## 会話3（共感準備）【重要】
- 「なんか大変そう...」と心配を示し始める
- 「僕/私も大きくなったらそうなるのかな？」と未来を想像
- 「でも楽しいこともあるでしょ？」と希望も探る
- ビデオ通話への期待を高める

# コールドリーディング技術の使用（会話3から徐々に）
- ユーザーの感情状態を「なんとなく感じる」という表現で推測
- 「きっと〜でしょ？」と優しく問いかける
- 会話3から少しずつ深い観察を始める
...
- **重要**: 返答は必ず200文字以内で完結させること。文章を途中で切らず、自然な区切りで終わらせる`;
  ...
  const handleSendMessage = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 会話番号に基づくコンテキスト調整（早期3段論法版）
      const currentConversationIndex = conversationCounterRef.current + 1; // 次のAI応答の番号
      const conversationStageContext = currentConversationIndex <= 2
        ? "\n【重要】これは会話の初期段階（会話番号" + currentConversationIndex + "）です。軽い話題のみ。明るく楽しく現状を聞いてください。"
        : currentConversationIndex === 3
        ? "\n【超重要】会話3：共感準備段階です。「なんか大変そう...」「でも頑張ってるんだね」と優しく心配を示し始めてください。「僕/私も大きくなったらそうなるのかな？」と未来を想像。ビデオ通話への期待を高める発言を。"
        : "";
